...
def test_policy_override_precedes_confidence():
    assert route_action(evaluated("increase_credit_limit", 0.99)) == "execute_high_risk_action"


def test_high_confidence_low_risk_auto_executes():
    assert route_action(evaluated("send_email", 0.90)) == "execute_low_risk_action"


def test_low_confidence_escalates():
    assert route_action(evaluated("send_email", 0.82)) == "execute_high_risk_action"
